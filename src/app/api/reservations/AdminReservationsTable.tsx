          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r._id} className="border-t">
                ...
                <Td>
                  <code>{r.places.join(" ")}</code>
                </Td>
                <Td>{r.total ? `${r.total} €` : "—"}</Td>
                <Td>
                  <div className="flex gap-2">
                    <button
                      onClick={() => doAction(r._id, "confirm")}
                      disabled={r.status === "confirmed"}
                      className="rounded-md bg-emerald-600 px-2 py-1 text-white disabled:opacity-40"
                    >
                      Confirmer
                    </button>
                    <button
                      onClick={() => doAction(r._id, "cancel")}
                      disabled={r.status === "cancelled"}
                      className="rounded-md bg-rose-600 px-2 py-1 text-white disabled:opacity-40"
                    >
                      Annuler
                    </button>
                  </div>
